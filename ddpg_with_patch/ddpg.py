...
from maze_env import FourRoomMazeEnv
...
def evaluate_and_record(agent, n_episodes, max_ep_len, results_dir, epoch=None):
    """
    Run n_episodes deterministic episodes, capture frames, and save one GIF
    per episode.  Returns the same stats dict as evaluate().

    A dedicated FourRoomMazeEnv(render_mode='rgb_array') is created and closed
    ...
    env = FourRoomMazeEnv(max_steps=max_ep_len, render_mode='rgb_array')
    returns, lengths, successes = [], [], []
...
def train(
    ...
    record_freq      = 10,      # save GIFs every N epochs (0 = never)
    results_dir      = 'results',
    # ── Reproducibility ───────────────────────────────────
    seed             = 0,
    device           = 'cpu',
    ...
    # ── Environments ──────────────────────────────────────────────────────────
    train_env = FourRoomMazeEnv(max_steps=max_ep_len)
    eval_env  = FourRoomMazeEnv(max_steps=max_ep_len)
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    print("=" * 75)
    print("  DDPG — 4-Room Maze Navigation")
    print(f"  obs_dim={obs_dim}  act_dim={act_dim}  device={device}")
    print(f"  epochs={epochs}  steps/epoch={steps_per_epoch}  "
          f"total={total_steps:,}")
    print(f"  start_steps={start_steps}  update_after={update_after}  "
          f"batch={batch_size}")
    print(f"  γ={gamma}  ρ={polyak}  σ_noise={act_noise}")
    print("=" * 75)
    print(f"  {'Epoch':>5} | {'Steps':>7} | {'Time':>6} | "
    ...
    for t in range(1, total_steps + 1):
        ...
        if t % steps_per_epoch == 0:
            ...
            # Deterministic evaluation — record GIFs on recording epochs
            if record_freq > 0 and epoch % record_freq == 0:
                eval_stats = evaluate_and_record(
                    agent, n_eval_episodes, max_ep_len, results_dir, epoch=epoch
                )
                print(f"  [rec] GIFs → {results_dir}/epoch_{epoch:04d}/")
            else:
...
if __name__ == '__main__':
    ...
    # ── Demo run (validates training loop, shows early dynamics) ──────────────
    # 10 epochs × 2 000 steps = 20 000 total env steps  (~5 min on CPU)
    # start_steps=2 000: first epoch is pure random exploration
    # Expect EvSucc ≈ 0.00 throughout — this run is too short to converge.
    # The purpose is to confirm losses are finite and logs are well-formed.
    print("\n" + "=" * 75)
    print("  Demo training run  (20 000 steps — not expected to converge)")
    print("  For full training (400 000 steps) run locally with GPU.")
    print("=" * 75 + "\n")

    #train(
    #    epochs           = 10,
    #    steps_per_epoch  = 2_000,
    #    start_steps      = 2_000,   # covers epoch 1 entirely
    #    update_after     = 500,
    #    update_every     = 1,
    #    batch_size       = 256,
    #    gamma            = 0.98,
    ...
    # ── Full training run (run locally, preferably with GPU) ──────────────────
    train(
         epochs           = 100,
         steps_per_epoch  = 4_000,
         start_steps      = 5_000,
